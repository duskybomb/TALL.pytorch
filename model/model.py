...
class TALL(nn.Module):
    ...
    def forward(self, visual_feature, sentence_embedding):
        transformed_clip = self.v2s_lt(visual_feature)
        transformed_clip_norm = F.normalize(transformed_clip, p=2, dim=1)

        transformed_sentence = self.s2s_lt(sentence_embedding)
        transformed_sentence_norm = F.normalize(transformed_sentence, p=2, dim=1)

        cross_modal_vector = self.cross_modal_comb(transformed_clip_norm, transformed_sentence_norm)

        cross_modal_vector = cross_modal_vector.unsqueeze(0).permute(0, 3, 1, 2)
        mid_output = self.fc1(cross_modal_vector)
        mid_output = F.relu(mid_output)
        sim_score_mat = self.fc2(mid_output).squeeze(0)
